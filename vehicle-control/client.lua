	...
	local distanceToVeh = GetDistanceBetweenCoords(GetEntityCoords(player), GetEntityCoords(vehicle), 1)
		if DoesEntityExist(vehicle) then
			if distanceToVeh <= lockDistance then
				...
				ShowNotification("Aracı kilitledin ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~.")
				TriggerEvent('lockLights')
				else
				SetVehicleDoorsLocked(vehicle,1)
				ShowNotification("Aracın kilidi açıldı ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~.")
				TriggerEvent('lockLights')
				end
			else
				ShowNotification("~r~Bunu yapmak için aracın yanında olmalısın.")
			end
		else
			ShowNotification("~r~Kayıtlı aracın yok.")
		end
	end)
...
AddEventHandler('save',function() 
	...
	if (IsPedSittingInAnyVehicle(player)) then 
		if  saved == true then
			...
			ShowNotification("Kayıtlı aracın ~r~silindi~w~.")
			saved = false
		else
			RemoveBlip(targetBlip)
			...
			ShowNotification("Bu ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~ artık senin~g~ kayıtlı ~w~aracın.")
			saved = true
		end
	end
end)
